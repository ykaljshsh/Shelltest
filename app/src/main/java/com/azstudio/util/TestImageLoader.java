...
/**
 * Created by YKA-SFB on 2017/3/10.
 */
...
public class TestImageLoader {
    ...
    // SD卡上图片储存地址主目录 SD卡目录/Android/data/包名/
    private String path = "";
    ...
    public Drawable loadDrawable(final String imageUrl,
        ...
        // 缓存中没有图像，则从网络上取出数据，并将取出的数据缓存到内存(SD卡)中
    ...
    // 下载失败时的手动重试下载方法（不检查缓存中是否存在）
    public Drawable loadImageFromUrl(final String imageUrl, final ImageCallback callback) {
        executorService.submit(new Runnable() {
            public void run() {
                try {
                    final Drawable drawable = Drawable.createFromStream(
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                if (iv.getImageMatrix() == null) {
                    iv.setImageResource(R.drawable.default_img); // 使用默认图片
                }
                // 如果缓存过就会从缓存中取出图像，ImageCallback接口中方法也不会被执行
                final Drawable cacheImage = loadDrawable(url,
                        new TestImageLoader.ImageCallback() {
    ...
    // 下载失败时的手动重试下载方法,对外调用
    public void LoadImageByMe(final String url, final ImageView iv) {
        if (iv.getImageMatrix() == null) {
            iv.setImageResource(R.drawable.default_img); // 使用默认图片
        }
        // 如果缓存过就会从缓存中取出图像，ImageCallback接口中方法也不会被执行
        Drawable cacheImage = loadImageFromUrl(url,
                new TestImageLoader.ImageCallback() {
                    // 请参见实现：如果第一次加载url时下面方法会执行
                    public void imageLoaded(Drawable imageDrawable) {
                        iv.setImageDrawable(imageDrawable);
    ...
    public void saveFile(Drawable dw, String url) {
        try {
            ...
            File file = new File(path + "/Images/" + fileNa);
            // 创建图片缓存文件夹
